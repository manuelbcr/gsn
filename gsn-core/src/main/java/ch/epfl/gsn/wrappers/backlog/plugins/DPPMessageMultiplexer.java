...
public class DPPMessageMultiplexer implements BackLogMessageListener {
    ...
    private final transient Logger logger = LoggerFactory.getLogger( DPPMessageMultiplexer.class);
	...
	public boolean messageRecv(int deviceID, BackLogMessage message) {
		try {
			int type = ch.epfl.gsn.wrappers.backlog.plugins.AbstractPlugin.toInteger(message.getPayload()[2]);
