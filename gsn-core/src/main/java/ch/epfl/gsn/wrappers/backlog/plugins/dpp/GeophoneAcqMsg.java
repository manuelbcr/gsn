...
public class GeophoneAcqMsg extends AbstractMsg {
	...
	private static DataField[] dataField = {
			...
			new DataField("TRG_SOURCE", "SMALLINT"),             /* Source of initial trigger. 0: external | 1: positive threshold, 2: negative threshold */
			new DataField("ADC_PGA", "SMALLINT"),                /* ADC PGA value. 0: PGA is off. */
			new DataField("ID", "BIGINT"),                       /* Acquire # Identifier */
			new DataField("ADC_SPS", "SMALLINT")                 /* ADC rate (sampling frequency), 0 = 1kHz, 1 = 500Hz, 2 = 250Hz, 3 = 125Hz */
			};
	...
	public Serializable[] receivePayload(ByteBuffer payload) throws Exception {
		...
		return new Serializable[]{start_time, first_time, samples, peak_pos_val, peak_pos_sample, peak_neg_val, peak_neg_sample, trg_count_pos, trg_count_neg, trg_last_pos_sample, trg_last_neg_sample, trg_gain, trg_th_pos, trg_th_neg, trg_source, adc_pga, id, adc_sps};
	}
