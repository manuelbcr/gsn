...
public class NodeInfoMsg extends AbstractMsg {
	...
	private static DataField[] dataField = {
			...
			new DataField("COMPILER_VER", "BIGINT"),		/* compiler version: human-readable (e.g. AAABBBCCC where A: major, B: minor, C: patch */
			...
			new DataField("FW_VER", "INTEGER"),				/* firmware version: human-readable (e.g. ABBCC where A: major, B: minor, C: patch */
			new DataField("SW_REV_ID", "BIGINT"),			/* repository revision number (GIT or SVN) */
			new DataField("CONFIG", "BIGINT")				/* bitfield for custom (application specific) compile-time configuration settings */
			};
	...
	public Serializable[] receivePayload(ByteBuffer payload) throws Exception {
		...
		return new Serializable[]{component_id, rst_flag, rst_cnt, mcu_desc, compiler_desc, compiler_ver, compile_date, fw_name, fw_ver, sw_rev_id, config};
	}
